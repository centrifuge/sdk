...
describe('utils/BigInt', () => {
  ...
  it('should perform arithmetic operations on Currency', () => {
    ...
    const e = new Currency(1000000000n, 6).mul(2n)
    expect(e.toString()).to.be.equal('2000')
    const f = new Currency(1000000n, 6).div(2000000n)
    expect(f.toDecimal().toString()).to.be.equal('0.5')
  })
  it('should convert float to Token and perform arithmetic operations', () => {
    ...
    const c = Token.fromFloat(1000000, 6).mul(2n)
    expect(c.toDecimal().toString()).to.be.equal('2')
  })
