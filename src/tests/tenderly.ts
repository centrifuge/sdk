...
export const forkTenderlyNetwork = async (chainId: number) => {
  try {
    ...
    return { forkedRpcUrl, vnetId: virtualNetwork.id }
  } catch (error) {
    throw error
  }
}

export const deleteTenderlyRpcEndpoint = async (vnetId: string) => {
  try {
    const tenderlyApi = `${TENDERLY_API}/account/${ACCOUNT_SLUG}/project/${PROJECT_SLUG}/vnets/${vnetId}`
    const response = await fetch(tenderlyApi, {
      method: 'DELETE',
      headers: {
