...
On mainnet, every indexer-reported deployment is checked against `KNOWN_DEPLOYMENTS` in
`src/config/deployments.ts`, generated by `pnpm gen:deployments` from the protocol repo's
`env/*.json` files (some entries are hand-patched ahead of a generator run — see the
comments on those entries). A `pnpm gen:deployments` run drops any hand-patched field that
isn't in a protocol env file yet (e.g. entry 14's `onOffRampFactory`), so check an entry's
comments before regenerating. On/off-ramp manager factory resolution is `onOffRampFactory ??
onOfframpManagerFactory`; either field is dropped if the chain's allowlist entry doesn't
cover it, so a new factory needs an allowlist entry before the SDK will use it.
