...
const publicClient = createClient({ transport: custom(mockProvider()) }).extend(() => ({
...
describe('Centrifuge', () => {
    ...
    describe('Queries', () => {
        ...
        it('should fetch the value of an asset in relation to another one', async () => {
            const quote = await context.centrifuge._getQuote((await context.centrifuge._protocolAddresses(chainId)).identityValuation, Balance.fromFloat(100, 6), assetId, AssetId.fromIso(840), chainId);
    ...
    describe('Query', () => {
        ...
        it('should memoize the observable by key', async () => {
            ...
            const query2 = context.centrifuge._query(['key'], () => of(object2).pipe(tap(() => (secondObservableCalled = true))));
        ...
        it('should cache the latest value by default', async () => {
            ...
            const query1 = context.centrifuge._query([Math.random()], () => defer(() => {
                subscribedTimes++;
                return lazy(1);
            }));
            const value1 = await query1;
        ...
        it("shouldn't cache the latest value when `cache` is `false` on the Centrifuge instance", async () => {
            ...
            let value = 0;
            const query1 = centrifuge._query([Math.random()], () => defer(() => {
                value++;
                return lazy(value);
            }));
            const value1 = await query1;
        ...
        it('should cache nested queries', async () => {
            ...
            const query3 = context.centrifuge._query(null, () => combineLatest([query1, query2]).pipe(map(([v1, v2]) => v1 + v2)));
        ...
        it.skip('should recreate the shared observable when the cached value is expired', async () => {
            let i = 0;
            const query1 = context.centrifuge._query([Math.random()], () => defer(async function* () {
    ...
    describe('Transact', () => {
        ...
        it('should try to switch chains when the signer is connected to a different one', async () => {
            ...
            const statuses = await firstValueFrom(tx.pipe(take(2), toArray()));
        ...
        it("shouldn't try to switch chains when the signer is connected to the right chain", async () => {
            ...
            const status = await firstValueFrom(tx);
        ...
        it('should emit status updates', async () => {
            ...
            expect(statuses).to.eql([
                { id: statuses[0].id, type: 'SigningTransaction', title: 'Test' },
                { id: statuses[0].id, type: 'TransactionPending', title: 'Test', hash: '0x1' },
                {
                    id: statuses[0].id,
        ...
        it('should emit status updates for a sequence of transactions', async () => {
            ...
            expect(statuses).to.eql([
                ...
                { id: statuses[2].id, type: 'SigningTransaction', title: 'Test' },
                { id: statuses[2].id, type: 'TransactionPending', title: 'Test', hash: '0x2' },
                {
                    id: statuses[2].id,
                    type: 'TransactionConfirmed',
                    title: 'Test',
...
function lazy(value, t = 10) {
    return new Promise((res) => setTimeout(() => res(value), t));
}
...
//# sourceMappingURL=Centrifuge.test.js.map