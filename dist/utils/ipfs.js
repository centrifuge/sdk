export function createPinning(pinningApiUrl) {
    return {
        pinFile: (b64URI) => pinToApi(`${pinningApiUrl}/pinFile`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ uri: b64URI }),
        }),
        pinJson: (json) => pinToApi(`${pinningApiUrl}/pinJson`, {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
...
export async function pinToApi(url, reqInit) {
...
export function getUrlFromHash(uriOrHash, ipfsGateway) {
    try {
        let newUrl;
