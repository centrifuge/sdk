...
describe('Solana Invest', () => {
    let sdk;
    let mockWallet;
    let mockKeypair;
    beforeEach(() => {
        // Initialize SDK with Solana support (testnet)
        sdk = new Centrifuge({
            environment: 'testnet',
            solana: {
        ...
        mockWallet = {
            ...
            signTransaction: async (tx) => {
                // In a real scenario, this would show a wallet popup
    ...
    describe('Validation', () => {
        ...
        it('should throw error if pool is not configured', async () => {
            const amount = Balance.fromFloat(100, 6);
            // Use a ShareClassId that doesn't exist in our mapping
            const unconfiguredShareClassId = new ShareClassId('0xffffffffffffffffffffffffffffffffffffffff');
            try {
    ...
    describe('Transaction Flow', () => {
        it('should emit status updates in correct order', (done) => {
            // Note: This test will fail without a real pool configuration
            // It's here to demonstrate the expected flow
            const amount = Balance.fromFloat(100, 6);
            const shareClassId = new ShareClassId('0x1234567890abcdef1234567890abcdef12345678');
            const statuses = [];
            ...
            sdk
                .solana.invest(amount, shareClassId, mockWallet)
                .subscribe({
                next: (status) => {
    ...
    describe('Balance Class Integration', () => {
        it('should correctly convert Balance to USDC amount', () => {
            ...
            expect(amount1.toBigInt()).to.equal(1000500000n);
            const amount2 = Balance.fromFloat(0.01, 6);
            expect(amount2.toBigInt()).to.equal(10000n);
            const amount3 = Balance.fromFloat(1_000_000, 6);
            expect(amount3.toBigInt()).to.equal(1000000000000n);
        });
        it('should correctly display USDC amount', () => {
            const amount = new Balance(1234567890n, 6);
...
//# sourceMappingURL=invest.test.js.map