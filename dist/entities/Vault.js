...
export class Vault extends Entity {
     ...
     */
    details() {
        return this._query(['details'], () => combineLatest([this.isLinked(), this._isSyncDeposit(), this._investmentCurrency(), this._shareCurrency()]).pipe(map(([isLinked, isSyncInvest, investmentCurrency, shareCurrency]) => ({
            pool: this.pool,
    ...
    investment(investor) {
        ...
        return this._query(['investment', investorAddress], () => combineLatest([
        ...
        ]).pipe(switchMap(([investmentCurrency, shareCurrency, addresses, restrictionManagerAddress, isSyncInvest, escrowAddress, isOperatorEnabled,]) => combineLatest([
            ...
            defer(async () => {
                ...
                const [isAllowedToInvest, maxDeposit, maxRedeem, investment, isAllowedToRedeem, [escrowTotal, escrowReserved],] = await Promise.all([
                ...
                return {
                    ...
                    claimableInvestCurrencyEquivalent: new Balance(isSyncInvest ? 0n : maxDeposit, investmentCurrency.decimals),
    ...
    increaseInvestOrder(amount) {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (investmentCurrency.decimals !== amount.decimals)
            ...
            if (needsApproval) {
                ...
                if (supportsPermit) {
                    try {
                        permit = yield* doSignMessage('Sign Permit', () => signPermit(ctx, investmentCurrency.address, spender, amount.toBigInt()));
                    }
                    catch (e) {
    ...
    cancelInvestOrder() {
        ...
        return this._transact(async function* (ctx) {
            ...
            yield* doTransaction('Cancel invest order', ctx, () => ctx.walletClient.writeContract({
                address: vaultRouter,
                abi: ABI.VaultRouter,
     ...
     */
    increaseRedeemOrder(sharesAmount) {
        const self = this;
        return this._transact(async function* (ctx) {
            ...
            if (investment.shareCurrency.decimals !== sharesAmount.decimals)
                throw new Error('Invalid amount decimals');
    ...
    cancelRedeemOrder() {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (investment.pendingRedeemShares.isZero())
                throw new Error('No order to cancel');
            yield* doTransaction('Cancel redeem order', ctx, () => ctx.walletClient.writeContract({
                address: vaultRouter,
                abi: ABI.VaultRouter,
    ...
    _allowance(owner) {
        return this._query(['allowance', owner.toLowerCase()], () => combineLatest([
            ...
            this._root._protocolAddresses(this.chainId),
            this._isSyncDeposit(),
        ]).pipe(switchMap(([currency, { vaultRouter }, isSyncDeposit]) => this._root
