...
export class Vault extends Entity {
    ...
    address;
    /** @internal */
    constructor(_root, network, shareClass, asset, address, assetId) {
     ...
     */
    details() {
        return this._query(['details'], () => combineLatest([this.isLinked(), this._isSyncDeposit(), this._investmentCurrency(), this._shareCurrency()]).pipe(map(([isLinked, isSyncDeposit, assetDetails, share]) => ({
            pool: this.pool,
     ...
     */
    isLinked() {
        return this._query(['linked'], () => this._root._protocolAddresses(this.chainId).pipe(switchMap(({ spoke }) => defer(async () => {
    ...
    investment(investor) {
        ...
        return this._query(['investment', investorAddress], () => combineLatest([
            ...
            defer(async () => {
                ...
                const [isAllowedToDeposit, maxDeposit, maxRedeem, investment, isAllowedToRedeem, [escrowTotal, escrowReserved],] = await Promise.all([
                ...
                const [maxMint, maxWithdraw, , , pendingDeposit, pendingRedeem, claimableCancelDepositAssets, claimableCancelRedeemShares, hasPendingCancelDepositRequest, hasPendingCancelRedeemRequest,] = investment;
                ...
                filter: (events) => events.some((event) => event.args.receiver?.toLowerCase() === investorAddress ||
    ...
    syncDeposit(amount) {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (!isSyncDeposit)
                throw new Error('Vault does not support synchronous deposits');
            ...
            if (!isAllowedToDeposit)
                throw new Error('Not allowed to deposit');
            if (asset.decimals !== amount.decimals)
            ...
            if (needsApproval) {
                if (supportsPermit) {
                    try {
                        permit = yield* doSignMessage('Sign Permit', () => signPermit(ctx, asset.address, spender, amount.toBigInt()));
                    }
                    catch (e) {
                ...
                if (!permit) {
                    ...
                    yield* doTransaction('Approve', ctx, () => ctx.walletClient.writeContract({
    ...
    asyncDeposit(amount) {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (isSyncDeposit)
                throw new Error('Vault does not support asynchronous deposits');
            const { asset, assetBalance, assetAllowance, isAllowedToDeposit } = investment;
            const supportsPermit = asset.supportsPermit && signingAddressCode === undefined;
            const needsApproval = assetAllowance.lt(amount);
            if (!isAllowedToDeposit)
    ...
    cancelDepositRequest() {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (investment.pendingDepositAssets.isZero())
                throw new Error('No order to cancel');
            yield* doTransaction('Cancel deposit request', ctx, () => ctx.walletClient.writeContract({
                address: vaultRouter,
                abi: ABI.VaultRouter,
    ...
    asyncRedeem(sharesAmount) {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (!investment.isAllowedToRedeem)
                throw new Error('Not allowed to redeem');
            if (investment.share.decimals !== sharesAmount.decimals)
                throw new Error('Invalid amount decimals');
            if (sharesAmount.gt(investment.shareBalance))
                throw new Error('Insufficient balance');
            if (!sharesAmount.gt(0n))
                throw new Error('Order amount must be greater than 0');
            if (isOperator) {
                yield* doTransaction('Redeem', ctx, () => ctx.walletClient.writeContract({
                    address: vaultRouter,
                    abi: ABI.VaultRouter,
    ...
    cancelRedeemRequest() {
        ...
        return this._transact(async function* (ctx) {
            ...
            if (investment.pendingRedeemShares.isZero())
                throw new Error('No order to cancel');
            yield* doTransaction('Cancel redeem request', ctx, () => ctx.walletClient.writeContract({
                address: vaultRouter,
                abi: ABI.VaultRouter,
        ...
        return this._transact(async function* (ctx) {
            ...
            if (investment.claimableCancelDepositAssets.gt(0n)) {
                ...
                if (isOperator) {
                    ...
                    yield* doTransaction('Claim', ctx, () => ctx.walletClient.writeContract({
                        address: vaultRouter,
                        abi: ABI.VaultRouter,
                        functionName: 'multicall',
     ...
     * @internal
     */
    _isOperator(investorAddress) {
        return this._query(['isOperator', investorAddress], () => defer(() => this._root
    ...
    /** @internal */
    _isSyncDeposit() {
        return this._query(['isSyncDeposit'], () => defer(() => this._root
            .getClient(this.chainId)
            .readContract({
            address: this.address,
            abi: ABI.AsyncVault,
     ...
     */
    _restrictionManager() {
        return this._query(['restrictionManager'], () => this.network._share(this.shareClass.id).pipe(switchMap((share) => this._root.getClient(this.chainId).readContract({
            address: share,
            abi: ABI.Currency,
            functionName: 'hook',
        }))));
     ...
     */
    _allowance(owner) {
        return this._query(['allowance', owner.toLowerCase()], () => combineLatest([
            this._investmentCurrency(),
            this._root._protocolAddresses(this.chainId),
            this._isSyncDeposit(),
        ]).pipe(switchMap(([asset, { vaultRouter }, isSyncDeposit]) => this._root
