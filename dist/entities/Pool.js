...
export class Pool extends Entity {
     ...
     */
    details() {
        return this._query(['poolDetails', this.id.toString()], () => combineLatest([this.metadata(), this.shareClasses(), this.shareClassesDetails(), this.currency()]).pipe(map(([metadata, shareClasses, shareClassesDetails, currency]) => {
            return {
                id: this.id,
                currency,
                metadata,
    ...
    metadata() {
        return this._query(['metadata', this.id.toString()], () => this._root._protocolAddresses(this.chainId).pipe(switchMap(({ hubRegistry }) => defer(() => {
            ...
            return this._root._queryIPFS(fromHex(hash, 'string'));
        }), catchError((error) => {
    ...
    shareClasses() {
        return this._query(['shareClasses'], () => this._shareClassIds().pipe(map((scIds) => scIds.map((scId) => new ShareClass(this._root, this, scId.raw)))));
    }
    shareClass(scId) {
        return this._query(['shareClass', scId.toString()], () => this.shareClasses().pipe(map((shareClasses) => {
            const shareClass = shareClasses.find((sc) => sc.id.equals(scId));
            if (!shareClass)
                throw new Error(`Share class ${scId} not found`);
            return shareClass;
        })));
    }
    shareClassesDetails() {
        return this._query(['shareClassesDetails', this.id.toString()], () => {
            return this.shareClasses().pipe(switchMap((shareClasses) => {
    ...
    poolManagers() {
        return this._query(['poolManagers', this.id.toString()], () => {
            return this._managers().pipe(map((managers) => {
                return managers
    ...
    balanceSheetManagers() {
        return this._query(null, () => {
            return combineLatest([this._managers(), this._root._protocolAddresses(this.chainId)]).pipe(map(([managers, { asyncRequestManager, syncManager }]) => {
                return managers
     ...
     */
    isPoolManager(address) {
        const addr = address.toLowerCase();
        return this._query(['isPoolManager', this.id.toString(), addr], () => {
            return this.poolManagers().pipe(map((managers) => {
     ...
     * @param address - The address to check
     */
    isBalanceSheetManager(chainId, address) {
        const addr = address.toLowerCase();
        return this._query(['isBalanceSheetManager', chainId, this.id.toString(), addr], () => {
            return this.balanceSheetManagers().pipe(map((managers) => {
        ...
        return this._query(['poolActiveNetworks', this.id.toString()], () => {
            return this.networks().pipe(switchMap((networks) => {
                if (networks.length === 0)
                    return of([]);
                return combineLatest(networks.map((network) => network.isActive().pipe(
                ...
                timeout({ first: 5000 }), catchError(() => {
                    return of(false);
                })))).pipe(map((isActive) => networks.filter((_, index) => isActive[index])));
            }));
        });
    }
    vault(chainId, scId, asset) {
        return this._query(['vault', chainId, scId.toString(), asset.toString().toLowerCase()], () => this.network(chainId).pipe(switchMap((network) => network.vault(scId, asset))));
    }
    /**
    ...
    update(metadataInput = null, updatedShareClasses, addedShareClasses = []) {
        ...
        return this._transact(async function* (ctx) {
            ...
            const shareClassesToBeUpdatedPerNetwork = new Map();
            networksDetails.forEach((details, index) => {
                const activeShareClasses = details.activeShareClasses || [];
                const shareClassesToUpdate = activeShareClasses.filter((asc) => shareClassesToBeUpdatedIds.has(asc.id.toString()));
            ...
            for (const [chainId, shareClasses] of shareClassesToBeUpdatedPerNetwork) {
                if (shareClasses.length > 0) {
                    ...
                    await Promise.all(shareClasses.map(async (sc) => {
                        batch.push(encodeFunctionData({
                            abi: ABI.Hub,
    ...
    updateBalanceSheetManagers(updates) {
        ...
        return this._transact(async function* (ctx) {
            ...
            const batch = updates.map(({ address, canManage }, i) => encodeFunctionData({
                abi: ABI.Hub,
     ...
     */
    _shareClassIds() {
        return this._query(['shareClasses', this.id.toString()], () => this._root._protocolAddresses(this.chainId).pipe(switchMap(({ shareClassManager }) => defer(async () => {
    ...
    /** @internal */
    _escrow() {
        return this._query(['escrow', this.id.toString()], () => this._root._protocolAddresses(this.chainId).pipe(switchMap(({ poolEscrowFactory }) => {
            return this._root.getClient(this.chainId).readContract({
    ...
    _managers() {
        return this._query(null, () => combineLatest([
            ...
            this._root._queryIndexer(`query ($poolId: BigInt!) {
        ...
        ]).pipe(map(([deployments, { poolManagers }]) => {
            ...
            return poolManagers.items.map((manager) => {
                ...
                return {
                    address: manager.address.toLowerCase(),
