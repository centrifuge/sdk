    ...
    describe.skip('approveRedeemsAndRevokeShares', () => {
        ...
        before(async () => {
            ...
            [, investment] = await Promise.all([
                ...
                firstValueFrom(vault.investment(investor).pipe(skipWhile((i) => !i.pendingInvestCurrency.eq(defaultAssetsAmount.toBigInt())))),
            ]);
            ...
            [, investment] = await Promise.all([
                ...
                firstValueFrom(vault.investment(investor).pipe(skipWhile((i) => !i.claimableInvestShares.eq(defaultSharesAmount.toBigInt())))),
            ]);
            context.tenderlyFork.impersonateAddress = investor;
            context.centrifuge.setSigner(context.tenderlyFork.signer);
            [, investment] = await Promise.all([
            ...
            [, investment, pendingAmounts] = await Promise.all([
                ...
                firstValueFrom(vault.investment(investor).pipe(skipWhile((i) => !i.pendingRedeemShares.eq(redeemShares.toBigInt())))),
                firstValueFrom(vault.shareClass
                    .pendingAmounts()
...
async function mint(asset, address) {
    ...
    await context.centrifuge._transact(async function* (ctx) {
        yield* doTransaction('Mint', ctx, async () => {
            return ctx.walletClient.writeContract({
                ...
                functionName: 'mint',
                args: [address, defaultAssetsAmount.toBigInt()],
            });
        });
    }, chainId);
}
//# sourceMappingURL=ShareClass.test.js.map