    ...
    describe.skip('approveRedeemsAndRevokeShares', () => {
        ...
        before(async () => {
            ...
            [, investment] = await Promise.all([
                ...
                firstValueFrom(vault.investment(investor).pipe(skipWhile((i) => !i.pendingDepositAssets.eq(defaultAssetsAmount.toBigInt())))),
            ]);
            ...
            [, investment] = await Promise.all([
                ...
                firstValueFrom(vault.investment(investor).pipe(skipWhile((i) => !i.claimableDepositShares.eq(defaultSharesAmount.toBigInt())))),
            ]);
            context.tenderlyFork.impersonateAddress = investor;
            context.centrifuge.setSigner(context.tenderlyFork.signer);
            [, investment] = await Promise.all([
