...
export class OnOffRampManager extends Entity {
     ...
     */
    setReceiver(assetId, receiver, enabled = true) {
        const self = this;
            ...
            yield* wrapTransaction(enabled ? 'Enable Receiver' : 'Disable Receiver', ctx, {
                ...
                data: encodeFunctionData({
                    ...
                    args: [
                        ...
                        encodePacked(['uint8', 'bytes32', 'uint128', 'bytes32', 'bool'], [3, toHex('offramp', { size: 32 }), assetId.raw, addressToBytes32(receiver), enabled]),
                        0n,
                    ],
                }),
            });
     ...
     */
    setRelayer(relayer, enabled = true) {
        const self = this;
        return this._transact(async function* (ctx) {
            const [id, { hub }] = await Promise.all([
                self._root.id(self.network.chainId),
            ...
            yield* wrapTransaction(enabled ? 'Enable Relayer' : 'Disable Relayer', ctx, {
                ...
                data: encodeFunctionData({
                    ...
                    args: [
                        ...
                        id,
                        addressToBytes32(self.onrampAddress),
                        encodePacked(['uint8', 'bytes32', 'uint128', 'bytes32', 'bool'], [3, toHex('relayer', { size: 32 }), 0n, addressToBytes32(relayer), enabled]),
                        0n,
                    ],
                }),
            });
