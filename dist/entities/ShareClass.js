...
export class ShareClass extends Entity {
     ...
     */
    details() {
        return this._query(['shareClassDetails'], () => combineLatest([
            this._metrics(),
            this._metadata(),
            this.navPerNetwork(),
            this.pool.currency(),
    ...
    deploymentPerNetwork() {
        return this._query(['deploymentPerNetwork'], () => this.pool.activeNetworks().pipe(switchMap((networks) => {
    ...
    navPerNetwork() {
        return this._query(['navPerNetwork'], () => this.pool.currency().pipe(switchMap((poolCurrency) => this._root._queryIndexer(`query ($scId: String!) {
    ...
    holders(options) {
        ...
        return this._query([
            ...
            const whitelistedQueries = tokenInstancePositions.map((position) => this._whitelistedInvestor({
    ...
    whitelistedHolders(options) {
        ...
        return this._query(['whitelistedHolders', this.id.raw, limit, offset], () => combineLatest([
            ...
            const positionQueries = whitelistedInvestors.map((investor) => this._tokenInstancePosition({
     ...
     */
    closedInvestments() {
        return this._query(['closedInvestments'], () => this._root._queryIndexer(`query ($scId: String!) {
        investOrders(where: { tokenId: $scId, issuedAt_not: null }, limit: 1000) {
          items {
            ...
            return data.investOrders.items.map((order) => ({
                investor: order.account.toLowerCase(),
                index: order.index,
     ...
     */
    closedRedemptions() {
        return this._query(['closedRedemptions'], () => this._root._queryIndexer(`query ($scId: String!) {
    ...
    _tokenInstancePositions(options) {
        ...
        return this._query([
            ...
            if (balance_gt !== undefined)
                whereConditions.push('balance_gt: $balance_gt');
            if (holderAddress)
                whereConditions.push('accountAddress_contains: $holderAddress');
            if (centrifugeIds)
                whereConditions.push('centrifugeId_in: $centrifugeIds');
            ...
            if (balance_gt !== undefined)
                queryParams.push('$balance_gt: BigInt');
            if (holderAddress)
                queryParams.push('$holderAddress: String');
            if (centrifugeIds)
                queryParams.push('$centrifugeIds: [String!]');
            ...
            if (balance_gt !== undefined)
                variables.balance_gt = balance_gt.toString();
            if (holderAddress)
                variables.holderAddress = holderAddress;
            if (centrifugeIds)
                variables.centrifugeIds = centrifugeIds;
            return this._root
                ._queryIndexer(`query (${queryParams.join(', ')}) {
                tokenInstancePositions(
    ...
    _whitelistedInvestors(options) {
        ...
        return this._query(['whitelistedInvestors', this.id.raw, limit, offset], () => this._root
            ._queryIndexer(`query ($tokenId: String!, $limit: Int!, $offset: Int!) {
    ...
    /** @internal */
    _whitelistedInvestor(params) {
        return this._query(['whitelistedInvestor', this.id.raw, params.accountAddress, params.centrifugeId, params.tokenId], () => this._root
            ._queryIndexer(`query ($accountAddress: String!, $centrifugeId: String!, $tokenId: String!) {
          whitelistedInvestor(
    ...
    /** @internal */
    _tokenInstancePosition(params) {
        return this._query(['tokenInstancePosition', this.id.raw, params.accountAddress, params.centrifugeId, params.tokenId], () => this._root
            ._queryIndexer(`query ($accountAddress: String!, $centrifugeId: String!, $tokenId: String!) {
