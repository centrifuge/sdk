...
describe.skip('Vault', () => {
    describe('Vault - Async', () => {
        ...
        it.skip('completes the invest/redeem flow', async () => {
            ...
            expect(result[3].title).to.equal('Approve');
            expect(result[6].type).to.equal('TransactionConfirmed');
            expect(result[6].title).to.equal('Invest');
        ...
        it("throws when placing an invest order larger than the users's balance", async () => {
            ...
            try {
                await lastValueFrom(vault.asyncDeposit(Balance.fromFloat(100000000000, 6)).pipe(tap(() => (emittedSigningStatus = true))));
            }
            catch (e) {
                error = e;
            }
