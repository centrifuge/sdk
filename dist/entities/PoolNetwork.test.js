...
describe.skip('PoolNetwork', () => {
    ...
    it.skip('deploys share classes and vaults', async () => {
        ...
        await context.centrifuge._transact(async function* (ctx) {
            yield* doTransaction('Add share class', ctx, async () => {
                return ctx.walletClient.writeContract({
                    ...
                    args: [poolId.raw, 'Test Share Class', 'TSC', '0x123'.padEnd(66, '0')],
                });
            });
        }, chainId);
        ...
        const result = await poolNetwork.deploy([{ id: scId, hook: freezeOnlyHook }], [{ shareClassId: scId, assetId, kind: 'syncDeposit' }]);
        ...
        const vault = new Vault(context.centrifuge, new PoolNetwork(context.centrifuge, new Pool(context.centrifuge, poolId.raw, chainId), chainId), details.activeShareClasses.find((sc) => sc.id.equals(scId)).shareClass, asset.address, vaultAddr, assetId);
    ...
    it.skip('disables vaults', async () => {
        ...
        const result = await poolNetwork.unlinkVaults([
            { shareClassId: scId, assetId, address: vaultAddr },
        ]);
        expect(result.type).to.equal('TransactionConfirmed');
    ...
    it.skip('links vaults', async () => {
        ...
        const unlinkResult = await poolNetwork.unlinkVaults([
            { shareClassId: scId, assetId, address: initialVaultAddr },
        ]);
